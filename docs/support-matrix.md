...
| Changing a token's bound key | Conditional on supporting key rotation; [RFC 9635 §6.1.1](https://www.rfc-editor.org/rfc/rfc9635.html#section-6.1.1) | Required target, distinct from changing the token value | **Partial**: the AS supports opt-in linked HTTP signatures, with permission checks and atomic nonce-pair reservation. [AS tests](../crates/gnap-as/tests/key_rotation.rs) exercise binding changes, replay and refusal, encoder binding, introspection and concurrent revocation. [Client–AS tests](../crates/gnap-as/tests/key_rotation/session.rs) connect the actual SDK roles in memory, checking successive rotations, refusal, sibling keys and grant continuation. This is not a deployed application flow. The client presents public PS256 JWKs by value and does not resolve initially explicit token bindings; the AS also accepts references through a custom resolver. Derived-token rebinding is refused. Application consumers and native Biscuit binding changes remain unverified or missing. |
