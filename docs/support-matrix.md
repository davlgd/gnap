...
| Imported-message diagnostics | Project tooling, not an RFC certification scheme | Required tool | **Partial**: [workbench tests](../apps/conformance-web/src/lib.rs) and HTTP acceptance cover selected checks. The four [RFC 9767 import profiles](../apps/conformance-web/tests/rs_imports.rs) inspect discovery, introspection requests/responses and RS errors independently of SDK message validators; registry data and digest utilities are shared. Declared context is not authenticated evidence. Actual RS authentication, live state and HTTP publication remain untested by imports; no certification verdict is issued. |
