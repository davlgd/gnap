...
| Changing a token's bound key | Conditional on supporting key rotation; [RFC 9635 §6.1.1](https://www.rfc-editor.org/rfc/rfc9635.html#section-6.1.1) | Required target, distinct from changing the token value | **Partial**: the AS supports opt-in linked HTTP signatures, with permission checks and atomic nonce-pair reservation. [AS tests](../crates/gnap-as/tests/key_rotation.rs) exercise binding changes, replay and refusal, per-token isolation, encoder binding, introspection and concurrent revocation. These are in-memory handler tests, not a client-session or deployed application flow. The default resolver accepts public PS256 JWKs; derived-token rebinding is refused. Client-session integration, application consumers and native Biscuit binding changes remain unverified or missing. |
