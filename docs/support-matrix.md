...
| RS-facing discovery | Optional API component; [RFC 9767 §§3, 3.1](https://www.rfc-editor.org/rfc/rfc9767.html#section-3) | Required RS consumer capability | **Implemented and tested** for the opaque profile: [SDK discovery tests](../crates/gnap-as/tests/rs.rs) and [HTTP consumer tests](../apps/delegation-demo/src/introspection_tests.rs) exercise `/.well-known/gnap-as-rs`, endpoint identity and separate RS authentication. Only `httpsig` is advertised; the token-format list is omitted because there is no registered opaque-format name. HTTP loopback is an explicit, labelled development deviation. |
| Authenticated token introspection | Optional API component; [RFC 9767 §§3.2–3.3](https://www.rfc-editor.org/rfc/rfc9767.html#section-3.2) define authentication and response rules | Required RS consumer capability | **Implemented and tested for the opaque profile**: [SDK tests](../crates/gnap-as/tests/rs.rs) cover RS proof, key binding, filtered rights, unknown parameters, rotation, revocation, storage failure and concurrent changes. [Demo tests](../apps/delegation-demo/src/introspection_tests.rs) exercise protected reads through real HTTP with a distinct RS key and no token-store access. Every read repeats discovery and introspection, without caching either result. An inactive result can mean the AS could not determine validity; it does not prove the token intrinsically invalid. No Biscuit introspection or independently deployed RS is supplied by this path. Resource registration is a separate capability below. |
