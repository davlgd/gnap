...
| Opaque reference access tokens | Conditional; RFC 9767 §2.2 permits AS/RS agreement on token representation | Existing test baseline | **Implemented and tested**: AS-issued values, authenticated HTTP introspection and retired-token rejection in the [demo tests](../apps/delegation-demo/src/introspection_tests.rs) and [HTTP acceptance script](../tools/smoke_ecosystem.py). The co-located RS uses a distinct key and does not access the AS store. These local tests do not establish independent-vendor interoperability or a deployed version. |
...
| Imported-message diagnostics | Project tooling, not an RFC certification scheme | Required tool | **Partial**: core message checks share SDK parsers; [workbench tests](../apps/conformance-web/src/lib.rs) and HTTP acceptance cover selected checks. [AS discovery checks](../apps/conformance-web/tests/discovery.rs) inspect HTTP context, JSON and exact endpoint identity without the SDK validator, and the four [RFC 9767 import profiles](../apps/conformance-web/tests/rs_imports.rs) inspect discovery, introspection requests/responses and RS errors independently of SDK message validators; registry data and digest utilities remain shared. Missing optional fields and unsupported URL interpretations are explicitly untested, and profile rules such as HTTP 200 and userinfo refusal are distinguished from GNAP requirements. Declared context is not authenticated evidence. Actual RS authentication, live state and HTTP publication remain untested by imports; no certification verdict is issued. |
