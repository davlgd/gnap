...
| `user_code` and `user_code_uri` start | C2 required; [§2.5.1](https://www.rfc-editor.org/rfc/rfc9635.html#section-2.5.1) | Required secondary-device milestone | **Partial**: the opt-in [SDK profile](secondary-device-interaction.md) issues eight-symbol codes, resolves normalized input and atomically retires code and redirect handle together. [Client–AS tests](../crates/gnap-as/tests/user_codes.rs) and [forced storage races](../crates/gnap-as/tests/user_codes/storage.rs) exercise the lifecycle without HTTP. The web code-entry/consent component, attempt limits and second-browser consumer remain missing; no hosted scenario or complete C2 profile is claimed. |
...
independent interoperability or new public deployment is claimed.
