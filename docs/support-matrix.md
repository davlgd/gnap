...
| Token expiration | `expires_in` is optional in [RFC 9635 §3.2.1](https://www.rfc-editor.org/rfc/rfc9635.html#section-3.2.1); validity checks apply when expiry is set | Required project policy for hosted examples | **Implemented and tested** for the reference-token path: optional [policy duration](../crates/gnap-as/src/policy.rs), issuance timestamps and fail-closed [time checks](../crates/gnap-as/tests/storage.rs). [Interop tests](../crates/gnap-as/tests/interop.rs) cover announced lifetime, renewal and unchanged records on refusal. The demo selects 1,200 seconds. The AS rechecks the grant revision before an active introspection response; the RS rechecks expiry after verifying the client proof. This does not guarantee that an active response arrives before a concurrent revocation. Refusing rotation after expiry is SDK policy, not a GNAP requirement. |
