...
| Imported-message diagnostics | Project tooling, not an RFC certification scheme | Required tool | **Partial**: [workbench tests](../apps/conformance-web/src/lib.rs) and HTTP acceptance cover selected checks. Six RFC 9767 profiles inspect [discovery, introspection and RS errors](../apps/conformance-web/tests/rs_imports.rs) plus [registration requests/responses](../apps/conformance-web/tests/resource_registration_imports.rs) independently of SDK message validators; registry data and digest utilities are shared. Declared context is not authenticated evidence. Actual RS authentication, format compatibility, registration state and HTTP publication remain untested by imports; no certification verdict is issued. |
