...
| Imported-message diagnostics | Project tooling, not an RFC certification scheme | Required tool | **Partial**: core message checks share SDK parsers; [workbench tests](../apps/conformance-web/src/lib.rs) and HTTP acceptance cover selected checks. [AS discovery checks](../apps/conformance-web/tests/discovery.rs) inspect HTTP context, JSON and exact endpoint identity without the SDK validator. Six RFC 9767 profiles inspect [discovery, introspection and RS errors](../apps/conformance-web/tests/rs_imports.rs) plus [registration requests/responses](../apps/conformance-web/tests/resource_registration_imports.rs) independently of SDK message validators; registry data and digest utilities remain shared. Missing optional fields and unsupported URL interpretations are explicitly untested, and profile rules such as HTTP 200 and userinfo refusal are distinguished from GNAP requirements. Declared context is not authenticated evidence. Actual RS authentication, format compatibility, registration state and HTTP publication remain untested by imports; no certification verdict is issued. |
