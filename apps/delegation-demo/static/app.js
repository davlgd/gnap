...
for (const button of document.querySelectorAll('[data-action]')) button.addEventListener('click', async () => {
  ...
  try {
    ...
    if (['start', 'start-multiple', 'expand', 'downscope'].includes(button.dataset.action) && data.state === 'pending' && data.interaction_uri) { const target = new URL(data.interaction_uri); if (target.origin !== location.origin) throw new Error('Unexpected interaction origin'); location.assign(target.href); return; }
