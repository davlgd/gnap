...
function render(data) {
  ...
  const enabled = {start: !['starting', 'failed'].includes(data.state), approve: data.state === 'pending', deny: data.state === 'pending', continue: continuable && wait === 0, read: data.token_present, 'read-archive': data.token_present, 'read-metadata': data.token_present, rotate: data.token_present, revoke: data.token_present, 'revoke-grant': data.continuation_open && wait === 0, downscope: changeable && (data.rights || []).length > 1, expand: changeable && !(data.rights || []).includes('synthetic-archive:read'), 'check-retired':data.retired_token_present};
