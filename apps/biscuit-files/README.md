...
This application's rotation changes the token value, not its presentation key.
The optional SDK key-rotation capability remains disabled here, and the encoder
refuses an explicit replacement binding rather than silently using the old key.
