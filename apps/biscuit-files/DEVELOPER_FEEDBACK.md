...
the AS had accepted a new presentation key. This consumer currently refuses
such a context and leaves key rotation disabled. Supporting it requires the
encoder, resource replay scope and client signer selection to use the current
binding together; value rotation alone does not establish that support.
The encoder guard is exercised directly by a unit test; protocol requests are
refused earlier while this capability is disabled.
